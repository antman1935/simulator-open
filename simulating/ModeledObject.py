...
class ModeledObject(SimObject):
    def __init__(self, model: torch.nn.Module, frames: int, input_references: list[Reference]):
        ...
        self.state_data = {"t": [i / float(frames) for i in range(frames)]}
    ...
    def step(self):
        ...
        # insert newest frame
        self.state_data.append([float(self.frames - 1) / (self.frames)] + [ref.get_normalized() for ref in self.input_references])

        X = torch.from_numpy(np.array([self.state_data]))
        X = torch.tensor(X.clone().detach().requires_grad_(True), dtype=torch.float)
        test_coeffs = X#torchcde.hermite_cubic_coefficients_with_backward_differences(X)
